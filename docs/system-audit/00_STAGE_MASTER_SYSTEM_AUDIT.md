# STAGE Master System Audit, Architecture & Design Document

**Location**: `docs/system-audit/00_STAGE_MASTER_SYSTEM_AUDIT.md`  
...
STAGE is an enterprise-grade visual feedback, review, and design refinement platform. It decouples **Review & Audit** from **Design Composition**:
...
- **`backend/routes/proxy.py`**:
  - HTML link rewriting, asset proxying, CSP header stripping, agent script injection (`stage-agent.js`), SSRF guard (`is_ssrf_safe`), and `PageVisit` recording.
...
| :--- | :--- |
| **`00_STAGE_MASTER_SYSTEM_AUDIT.md`** | **Master System Audit & Architecture Overview (This File)** |
