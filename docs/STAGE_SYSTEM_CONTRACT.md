# STAGE Authoritative System Reference Contract

This document serves as the authoritative reference contract for STAGE's visual feedback, QA review, and proxy-injected feedback platform. It details the exact specifications of the FastAPI backend, Next.js frontend, WebSocket realtime synchronization layer, database schemas, proxy routing middleware, and client Zustand state stores.

---

## 1. System Overview

STAGE is a collaborative QA review and visual feedback platform. It allows developers to import websites into isolated "Review Sessions," wherein public reviewers (or developers) can drop comment pins directly onto DOM nodes or canvas frames, modify elements' styles in real-time, capture screenshots, and sync presence indicators over a WebSocket layer.
...
* **Credentials Support:** `allow_credentials=True` (permits sending authentication cookies like `stagetoken` or `stagesessionid`).
...
2. **Interception & Context Extraction:** For all other request paths, the middleware determines the context Session ID using this search order:
   ...
   * Extracts from the cookie `stagesessionid`.
...
4. **Resolution Strategies & Asset Rewriting:**
   ...
   * **HTML Documents:** Rewritten on the fly by injecting the `stage-agent.js` and exposing variables (Session ID, Logical Target URL, base path) on `window.__STAGE__`. It also automatically enables `conservative_render_mode` on the Session model if Next.js signatures are detected.
...
STAGE uses two authentication systems: **Developer Auth** (JWT-based) and **Reviewer Auth** (session-scoped).
...
* **Frontend Storage:** Stored in a cookie named `stagetoken` and duplicated in Zustand `authStore` memory.
* **Injection:** Injected into every API request header as `Authorization: Bearer <token>`.
* **Edge Middleware Guard (`middleware.ts`):** Checks for the existence of `stagetoken` before routing to `/projects`, `/project`, `/dashboard`, `/settings`, or `/sessions`. Redirects back to `/login` if missing.
...
STAGE displays target websites inside an iframe by proxying asset requests.
...
1. Defines global variables: `window.__STAGE_SESSION_ID__`, `window.__STAGE_PROXY_ORIGIN__`, and `window.__STAGE_TARGET_URL__`.
...
STAGE captures screenshots in three ways:
...
STAGE exports QA session feedback in three formats:
