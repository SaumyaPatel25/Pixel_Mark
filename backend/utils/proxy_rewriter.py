...
def rewrite_html(
    ...
    for tag_name, attr in asset_tags:
        for el in soup.find_all(tag_name, **{attr: True}):
            ...
            # Rewrite asset URLs to proxy through the asset proxy
            el[attr] = f"/proxy/session/{session_id}/asset?url={urllib.parse.quote(abs_url)}"
    ...
    for el in soup.find_all(style=True):
        ...
        for u in urls:
            if u.startswith(("data:", "javascript:", "blob:")):
                continue
            abs_url = resolve_url(u)
            proxied_url = f"/proxy/session/{session_id}/asset?url={urllib.parse.quote(abs_url)}"
            # Replace precisely the URL source
