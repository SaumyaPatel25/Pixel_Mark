...
# PixelMark Phase 2 — AI IDE Prompts (Step by Step)
...
Project: PixelMark — a visual QA feedback platform.
Stack: Next.js 14 (App Router), TypeScript, Zustand.
Backend base URL: http://localhost:8765 (FastAPI)
Auth: JWT Bearer token stored in localStorage as "pm_token"
...
Project: PixelMark — Next.js 14, Zustand, JWT auth.
...
2. Persist key: "pm_auth" in localStorage
3. State shape:
...
4. Actions:
   - login(email, password):
       calls api.login → saves access_token to localStorage as "pm_token"
       calls api.me() → saves user to state
   - register(email, password, name):
       calls api.register → saves access_token to localStorage as "pm_token"
       calls api.me() → saves user to state
   - logout():
...
Project: PixelMark — Next.js 14 App Router, Tailwind CSS, dark theme.
...
3. PixelMark logo/title at top of card
...
Project: PixelMark — Next.js 14 App Router, Tailwind CSS.
...
Requirements:
1. "use client" directive
2. On mount: check localStorage for "pm_token"
...
3. Render a fixed left sidebar (w-56) with:
   - PixelMark brand name + tagline at top
   - Nav links:
     - /dashboard → "Dashboard"
...
Project: PixelMark — Next.js 14, Tailwind, api from @/lib/api.
...
Project: PixelMark — Zustand store for markers.
...
Project: PixelMark — Next.js 14, Tailwind, dark theme.
...
Project: PixelMark — Zustand store for the canvas layer.
Dependencies: api from @/lib/api
...
Project: PixelMark — Next.js 14, Tailwind, Zustand.
...
Project: PixelMark — Next.js 14 frontend.
...
Project: PixelMark — Next.js 14, Zustand.
