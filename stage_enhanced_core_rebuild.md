...
LAYER 2 — Audit Agent (injected JavaScript)
  ...
  - Communicates back to STAGE via postMessage
...
Project: STAGE — visual feedback SaaS.
...
- STAGE agent script is not injected into every page
...
D) Inject the STAGE Audit Agent script as the LAST tag before </body>:
   <script>
   window.__STAGE__ = {
...
- Add: X-STAGE-Session: {session_id}
- Add: X-STAGE-Page: {page_url}
...
Task:
Create backend/static/stage-agent.js
...
Store result in window.__STAGE__.rendererType
...
Store in window.__STAGE__.consoleErrors = []

NETWORK ERROR CAPTURE:

Patch window.XMLHttpRequest and window.fetch:
- Intercept every response
- If status >= 400: push to window.__STAGE__.networkErrors
...
document.addEventListener("click", async (e) => {
  ...
  const payload = {
    ...
    ...domCtx,
    ...viewCtx,
    console_errors: window.__STAGE__.consoleErrors.slice(-5),
    network_errors: window.__STAGE__.networkErrors.slice(-5),
...
Also fire STAGE_PAGE_LOAD on DOMContentLoaded:
window.parent.postMessage({
...
Rules:
- IIFE wrapped (no global pollution except window.__STAGE__)
...
Project: STAGE — visual feedback SaaS.
Context: Extension of stage-agent.js for Three.js and WebGL pages.

Task:
Add Three.js and WebGL marker capture to stage-agent.js.
...
1) Find the Three.js renderer and scene:
   ...
   - Store reference: window.__STAGE__.threeRenderer, window.__STAGE__.threeScene, window.__STAGE__.threeCamera
...
function discoverThreeScene() {
  ...
  for (const name of candidates) {
    ...
    if (obj && obj.isWebGLRenderer) {
      window.__STAGE__.threeRenderer = obj;
    }
    if (obj && obj.isScene) {
      window.__STAGE__.threeScene = obj;
    }
    if (obj && obj.isCamera) {
      window.__STAGE__.threeCamera = obj;
    }
    // nested: obj.renderer, obj.scene, obj.camera
    if (obj && obj.renderer?.isWebGLRenderer) window.__STAGE__.threeRenderer = obj.renderer;
    if (obj && obj.scene?.isScene) window.__STAGE__.threeScene = obj.scene;
    if (obj && obj.camera?.isCamera) window.__STAGE__.threeCamera = obj.camera;
  }
...
Project: STAGE — Next.js 14 App Router, Tailwind, Zustand.
...
Project: STAGE — FastAPI, SQLAlchemy 2.0, asyncpg, NeonDB.
...
Project: STAGE — FastAPI backend + Next.js frontend.
...
A) Proxy Engine tests
  ...
  - test_proxy_injects_agent_script: response HTML contains "stage-agent.js"
...
D) Agent script tests (unit-level logic in Python via js2py or just HTTP tests)
  - test_agent_script_served: GET /static/stage-agent.js returns 200 JS
