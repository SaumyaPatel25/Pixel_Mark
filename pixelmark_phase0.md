# PixelMark Phase 0 — Product Spine & Guardrails

## Purpose

Phase 0 has one job: lock down what PixelMark is, who it is for, and what must never change before a single line of new code is written. Every future phase references this document as a north star. If a feature idea contradicts anything in this document, it gets challenged before it gets built.
...
PixelMark is a zero-friction visual feedback platform that lets testers and clients annotate live websites by clicking directly on UI elements, instantly generating structured, developer-ready bug reports — with no extensions or accounts required for the reviewer.
...
> "PixelMark removes ambiguity from client feedback by turning clicks on a live site into structured, developer-ready issues — with zero friction for the reviewer."
...
| Tool | What it does | Why it fails for PixelMark's ICP |
|------|-------------|----------------------------------|
...
### PixelMark's defensible position
...
Here is the product spec for PixelMark:
...
2. Write 3 positioning sentences that make it impossible to confuse PixelMark with a screen recorder or screenshot tool.
...
**Validation test:** Read the 3 positioning sentences aloud. If they could describe Loom, BugHerd, or a screenshot tool — they are not specific enough. Iterate until they could only describe PixelMark.
...
Here is a full feature list for PixelMark 2.0:
...
Given PixelMark's positioning (zero-friction visual feedback, developer-ready output, no extension for reviewer), identify:
...
| 3 positioning sentences | ☐ | Must be PixelMark-exclusive |
...
1. In one sentence: what does PixelMark do that no other tool does?
