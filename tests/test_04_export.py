...
async def setup_data():
    ...
    async with httpx.AsyncClient(timeout=10) as client:
        ...
        # Project
        resp = await client.post(f"{BACKEND_URL}/projects/", json={"name": "Export Test Project"}, headers=headers)
        state["project_id"] = resp.json()["id"]
        
        # Session
        resp = await client.post(f"{BACKEND_URL}/sessions/", json={"project_id": state["project_id"], "title": "Export Session"}, headers=headers)
        ...
        for m in markers:
            await client.post(f"{BACKEND_URL}/markers/", json={"session_id": state["session_id"], **m}, headers=headers)

async def test_export_markdown_status():
    async with httpx.AsyncClient(timeout=10) as client:
        headers = {"Authorization": f"Bearer {state['token']}"}
        response = await client.get(f"{BACKEND_URL}/export/session/{state['session_id']}/markdown", headers=headers)
...
async def test_export_csv_status():
    async with httpx.AsyncClient(timeout=10) as client:
        headers = {"Authorization": f"Bearer {state['token']}"}
        response = await client.get(f"{BACKEND_URL}/export/session/{state['session_id']}/csv", headers=headers)
        assert response.status_code == 200
...
async def test_export_json_status():
    async with httpx.AsyncClient(timeout=10) as client:
        headers = {"Authorization": f"Bearer {state['token']}"}
        response = await client.get(f"{BACKEND_URL}/export/session/{state['session_id']}/json", headers=headers)
        assert response.status_code == 200
...
async def test_export_requires_auth():
    async with httpx.AsyncClient(timeout=10) as client:
        response = await client.get(f"{BACKEND_URL}/export/session/{state['session_id']}/markdown")
