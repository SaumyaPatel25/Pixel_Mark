...
3. test_frontend_accessible
   GET {VERCEL_URL}
   Assert: status 200
   Assert: response contains "STAGE" or "stage" (case insensitive)
...
Use a unique email per test run: f"qatest_{uuid.uuid4().hex[:6]}@stage.dev"
...
1. test_login_page_renders
   ...
   Assert: page title or h1 contains "STAGE" or "Sign in"
...
7. test_logout_flow
   ...
   Assert: localStorage "stagetoken" is removed
